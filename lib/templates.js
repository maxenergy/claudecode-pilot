...
function copyAllTemplates(targetDir, variables = {}) {
  ...
  const commandTemplates = [
    ...
    { source: 'developer-auto.md.template', target: 'developer-auto.md' },
    { source: 'developer-yolo.md.template', target: 'developer-yolo.md' },
  ...
  const taskJsSource = path.join(__dirname, '..', '.claude', 'tools', 'task.js');
  ...
  if (fileExists(taskJsSource)) {
    ...
    const oldTaskJsSource = path.join(__dirname, '..', '.claude-pilot', 'tools', 'task.js');
    if (fileExists(oldTaskJsSource)) {
      fs.copyFileSync(oldTaskJsSource, taskJsTarget);
    }
  }
}
